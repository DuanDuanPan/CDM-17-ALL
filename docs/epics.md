...
**When** 在另一个图（或当前图其他位置）打开模板库
**Then** 可以预览并拖拽该模板到画布中
**And** 插入的模板应保留原有的结构、数据和样式
...
**Given** 画布包含至少一个节点
**When** 按下 `Cmd/Ctrl + 0`
**Then** 画布应缩放到"适应屏幕"，显示全部节点（缩放不超过 100%，空画布不处理）
...
**And** 所有缩放/平移过程应有平滑动画（遵守 prefers-reduced-motion）
**And** 右下角显示当前缩放百分比，点击可重置到 100%
...
**And** 只显示该节点及其所有后代节点
**And** 切换应遵守 `prefers-reduced-motion`

**Given** 用户已下钻到某个深层节点
**When** 查看画布顶部
**Then** 应显示面包屑路径（根 → 父节点 → 当前节点）
**When** 点击面包屑中的任意层级
...
**Given** 多人协作编辑同一图谱
**When** 任一用户在子图视图中添加/修改/删除节点
**Then** 其他用户应能实时看到数据变更（即使在不同视图层级）
**And** 下钻路径是本地视图状态，不在客户端间同步
...
### Story 8.14: 智能折叠 (Smart Collapse)
...
**Note** 任务产出物默认为输出，符合交付逻辑

#### PBS视图 - 选择关联类型上传

**Given** 数据资源库 Drawer 处于 "PBS视图" Tab，已选中某个 PBS 节点
**When** 点击"上传"按钮并选择文件
**Then** 应弹出关联类型选择对话框（输入/输出/参考）
**When** 选择关联类型并确认
**Then** 文件上传完成后与选中 PBS 节点建立指定类型的关联
...
**And** 投影规则：对每个语义节点（PBS/TASK），在原始 parent 链上找到最近的语义祖先作为显示父节点；若无语义祖先则作为根节点

#### 节点 Breadcrumb 溯源

**Given** 用户选中一个节点
**When** 右侧面板顶部显示 breadcrumb
**Then** breadcrumb 应从**图谱根节点**到当前节点显示完整路径（含被隐藏的 DATA/ORDINARY 节点）
...
**Given** 节点视图处于激活状态
**When** 使用节点搜索
**Then** 作用域为投影后的 PBS/TASK 节点集合，过滤/高亮命中，自动展开命中路径
**When** 使用资产搜索
**Then** 作用域为数据资源库内的资产，结果可查看溯源并一键定位到关联节点
