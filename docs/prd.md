...
| R29 | 数据管理与工业格式预览          | Should | 独立数据资源库 Drawer（Cmd+D，宽度60-70%）；支持 节点（PBS+任务）/文件夹 两种组织视图（节点视图仅展示图谱中 PBS/TASK 子树片段，路径用 breadcrumb 呈现）；支持节点搜索与资产搜索；支持 STEP/IGES/glTF/网格(STL)/云图(VTK) 轻量化预览（Online3DViewer）；过程数据（任务交付物/数据节点）自动汇聚；节点-数据双向关联；MVP 阶段暂忽略权限 | R1,R17    |
